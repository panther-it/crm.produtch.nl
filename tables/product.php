...
if (isset($_GET["id"]))
{
    $form->datasource = SqlProducts::query(Settings::ASFORM, isset($id) ? "p.id=" . $id : FALSE);
...
if (isset($id)) 
{
    ...
    {
	case "GROUP":
		$tabs->Groups= new ProductsGrid("pp.parent=" . $id . " AND p.type='GROUP'");
	...
	//unset($tabs->Products->fields["info_uri"]);
		...
		$tabs->Products = new ProductsGrid("pp.parent=" . $id . " AND p.type='PRODUCT'");
	...
	case "PRODUCT":
		$tabs->Features = new ProductsGrid("pp.parent=" . $id . " AND p.type='FEATURE'");
		...
		break;
	case "FEATURE":
		$tabs->Values= new ProductsGrid("pp.parent=" . $id . " AND p.type='VALUE'");
